...
for(int i = 0; i < POBLACION; i++) {
	...
	while(generacion<MAX_NUM_GEN && fabs(fitness_tot1-fitness_tot2) > 0.000001)
	{
	  //while(population[POBLACION].fitness<=2.85){
      generacion++;
      seleccion(newpopulation, population, POBLACION);

	//printf("Esto escribo %lf\n",fitness_tot1);
     
	fitness_tot1 = fitness_tot2;
	fitness_tot2 = fit;
	//printf("Fit %lf\n ",fit);
	//printf("Fittot2 %lf\n ",fitness_tot2);
	//printf("Fittot1 %lf\n ",fitness_tot1);    
	//printf("resta %lf\n ", fabs(fitness_tot1-fitness_tot2));
...
//	}//fin for 0
	...
	// esto lo agregue yo
	printf("Valor real convertido en el rango del mejor individuo: %lf\n",population[POBLACION-1].convertido);
	printf("Cantidad de ejecuciones de la funcion objetivo: %d\n",fobjetivo);
